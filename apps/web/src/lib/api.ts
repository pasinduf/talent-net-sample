...
export function createAuthApi(token: string) {
  ...
  return {
    ...
    publishJob: (id: string) =>
      fetch<any>(`/jobs/${id}/publish`, { method: 'POST' }),
    pauseJob: (id: string) =>
      fetch<any>(`/jobs/${id}/pause`, { method: 'POST' }),
    closeJob: (id: string) =>
      fetch<any>(`/jobs/${id}/close`, { method: 'POST' }),
    duplicateJob: (id: string) =>
      fetch<any>(`/jobs/${id}/duplicate`, { method: 'POST' }),
    deleteJob: (id: string) =>
      fetch<any>(`/jobs/${id}`, { method: 'DELETE' }),

    // Scoring
    getScoringConfig: (jobId: string) =>
      fetch<any>(`/jobs/${jobId}/scoring`),
    ...
    addDimension: (jobId: string, body: unknown) =>
      fetch<any>(`/jobs/${jobId}/scoring/dimensions`, { method: 'POST', body: JSON.stringify(body) }),
    updateDimension: (jobId: string, dimId: string, body: unknown) =>
      fetch<any>(`/jobs/${jobId}/scoring/dimensions/${dimId}`, { method: 'PATCH', body: JSON.stringify(body) }),
    ...
    addKnockoutRule: (jobId: string, body: unknown) =>
      fetch<any>(`/jobs/${jobId}/scoring/knockout-rules`, { method: 'POST', body: JSON.stringify(body) }),
    removeKnockoutRule: (jobId: string, ruleId: string) =>
      fetch<any>(`/jobs/${jobId}/scoring/knockout-rules/${ruleId}`, { method: 'DELETE' }),
    validateScoringConfig: (jobId: string) =>
      fetch<any>(`/jobs/${jobId}/scoring/validate`),
